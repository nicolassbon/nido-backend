---
name: aspnet-core
description: Implement and configure ASP.NET Core web applications using current official guidance for app model selection, middleware, endpoints, authentication, authorization, configuration, and hosting.
---
...
- Implementing or refactoring ASP.NET Core application code
- Configuring middleware, routing, DI, auth, OpenAPI, logging, health checks, or hosting
- Choosing between controllers and minimal APIs for a concrete implementation task
- Working on framework-specific behavior in `Program.cs`, endpoints, controllers, or service registration

## Do not use this skill when

- Debating high-level project architecture or writing ADRs; use `dotnet-architect`
- Reviewing overall code design quality after implementation; use `dotnet-design-pattern-review`
- Applying everyday project-specific backend conventions where `dotnet-patterns` is enough on its own
