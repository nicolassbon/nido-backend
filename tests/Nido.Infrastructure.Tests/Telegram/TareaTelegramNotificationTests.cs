...
public sealed class TareaTelegramNotificationTests : IAsyncLifetime
{
    ...
    public async Task InitializeAsync()
    {
        ...
        _sut = new TareaRepository(_db, _fakeBatcher, NullLogger<TareaRepository>.Instance);
    }
    ...
    [Fact]
    public async Task CreateAsync_WhenTelegramLinkMembershipIsStale_DoesNotEnqueueAndUnpairsLink()
    {
        var (hogarId, creatorId, assignedId, chatId) = await SeedHogarAndUsersAsync(linkTelegram: true);

        var membership = await _db.MiembrosHogars.SingleAsync(x => x.UsuarioId == assignedId && x.HogarId == hogarId);
        ...
        var link = await _db.TelegramChatLinks.AsNoTracking().SingleAsync(x => x.ChatId == chatId);
    ...
    [Fact]
    public async Task AsignarAsync_WhenTelegramLinkMembershipIsStale_DoesNotEnqueueAndUnpairsLink()
    {
        var (hogarId, creatorId, assignedId, chatId) = await SeedHogarAndUsersAsync(linkTelegram: true);
        var taskResult = await _sut.CreateAsync(hogarId, creatorId, "Reassign Task", null, null, null, CancellationToken.None);
        _fakeBatcher.EnqueuedEvents.Clear();
    ...
    [Fact]
    public async Task CreateAsync_WhenTelegramBatcherFails_DoesNotFailPrimaryTaskFlow()
    {
        var (hogarId, creatorId, assignedId, _) = await SeedHogarAndUsersAsync(linkTelegram: true);
        _fakeBatcher.ExceptionToThrow = new InvalidOperationException("boom");

        var result = await _sut.CreateAsync(
            hogarId,
            creatorId,
            "Task With Telegram Failure",
            null,
            null,
            assignedId,
            CancellationToken.None);

        Assert.Equal("Task With Telegram Failure", result.Titulo);

        var storedTask = await _db.Tareas.AsNoTracking().SingleAsync(x => x.Id == result.Id);
        Assert.Equal("Task With Telegram Failure", storedTask.Titulo);
    }
