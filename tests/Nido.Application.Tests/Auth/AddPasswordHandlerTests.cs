...
public sealed class AddPasswordHandlerTests
{
    [Fact]
    public async Task Handle_ForUserWithoutPassword_SetsPassword()
    {
        var userId = Guid.NewGuid();
        var repository = new FakeAuthRepository
        {
            User = new User(userId, "No Password User", "no-password@test.com", null, "google", "google-id")
        };
        var handler = new AddPasswordHandler(repository, new FakePasswordHasher());

        var result = await handler.Handle(
            new AddPasswordCommand(userId, "NewPassword123!", "NewPassword123!"),
    ...
    [Fact]
    public async Task Handle_ForUserWithExistingPassword_ThrowsPasswordAlreadySet()
    {
        var repository = new FakeAuthRepository
        {
            User = new User(Guid.NewGuid(), "Local User", "local-user@test.com", "hashed:CurrentPassword123!", null, null)
        };
        var handler = new AddPasswordHandler(repository, new FakePasswordHasher());

        var exception = await Assert.ThrowsAsync<PasswordAlreadySetException>(() =>
            handler.Handle(
                new AddPasswordCommand(repository.User.Id, "NewPassword123!", "NewPassword123!"),
                CancellationToken.None));

        Assert.Equal("PASSWORD_ALREADY_SET", exception.Code);
        Assert.Equal("This account already has a password.", exception.Message);
    ...
    public async Task Handle_WithConfirmationMismatch_ThrowsValidationError()
    {
        var repository = new FakeAuthRepository
        {
            User = new User(Guid.NewGuid(), "No Password User", "no-password@test.com", null, "google", "google-id")
        };
        var handler = new AddPasswordHandler(repository, new FakePasswordHasher());

        var exception = await Assert.ThrowsAsync<InvalidPasswordException>(() =>
            handler.Handle(
                new AddPasswordCommand(repository.User.Id, "NewPassword123!", "DifferentPassword123!"),
                CancellationToken.None));
    ...
    public async Task Handle_WithWeakPassword_ThrowsWeakPassword()
    {
        ...
        var exception = await Assert.ThrowsAsync<WeakPasswordException>(() =>
            handler.Handle(
                new AddPasswordCommand(repository.User.Id, "weak", "weak"),
                CancellationToken.None));
