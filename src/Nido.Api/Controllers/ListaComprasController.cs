...
public sealed class ListaComprasController : ControllerBase
{
    ...
    private static ListaCompraItemResponse ToResponse(ListaCompraItemResult item)
        => new(item.Id, item.ProductoId, item.Nombre, item.Cantidad, item.Unidad, item.Comprado, item.CompradoEn, item.Orden, item.CategoriaNombre, item.IconoSvg, item.Icono);
